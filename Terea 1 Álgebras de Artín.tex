...
Los demás comandos realizan algo similar, pero para familias finitas y para cuando se desea o no se desea expecificar un superíndice.
\end{comment}
\newcommand{\arbtfam}[3]{
...
%Da la notación para el generado de un conjunto, por ejemplo <A>, con los símbolos < y > ajustados al tamaño del argumento.
\newcommand{\gengroup}[1]{
    \left< #1\right>
}

%Este es para el estabilizador, en el contexto de acciones de grupos. Ahora mismo no es necesario.
\newcommand{\stblzer}[2]{
    St_{#1}\lrprth{#2}
}
%Este coloca [] a cada lado del argumento, por ejemplo para escribir el conmutador de dos elementos.
\newcommand{\cmmttr}[1]{
    \left[#1,#1\right]
}

%Este permite escribir el indice de un grupo G sobre de un subgrupo H, o algo similar, así: $\grpindx{G}{H}$
\newcommand{\grpindx}[2]{
    \left[#1:#2\right]
}
%Este sirve para escribir el máximo común divisor de dos elementos, y el siguiente comando para el mínimo común múltiplo.
\newcommand{\grtcd}[2]{
    mcd\lrprth{#1,#2}
}
\newcommand{\lsttcm}[2]{
